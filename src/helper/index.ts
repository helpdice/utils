...
export function fetchCountryAPI(country ?: string | null, state ?: string | null) {
    const Countries: any[] = countries as any[]; 
    if (country) {
        const _country = Countries.find((cnty: { name: any; }) => cnty.name === country);
    ...
    return Countries.map((cnty: { name: any; iso3: any; currency: any; phone_code: any; }) => ({
        name: cnty.name,
        iso: cnty.iso3,
